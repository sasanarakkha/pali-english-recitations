...
% TODO decide whether uncompressed versions are needed at all. if the publisher inserts them manually with the image file separately and not with it being embedded in the pdf then no need. it can just be a digital conditional then.
\ifdigitalversion
...
{%
  ...
  \ifasixversion\fontsize{11}{17}\selectfont\fi%
  \ifasixversion\fontsize{8}{12}\selectfont\fi%
  ...
  {\centering
    ...
    \ifasixversion{\fontsize{18}{12}\selectfont \ccbyncnd}\fi
